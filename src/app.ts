...
class App {
    ...
    private async prepare(absPath: string): Promise<void> {
        // .tex is converted to markdown in-process (no TeX engine / no latex
        // grammar load). Markdown still needs code-block grammars prewarmed.
        if (docKind(absPath) === "tex") return;
    ...
    private async showViewer(absPath: string): Promise<void> {
        ...
        await this.prepare(absPath);
        const kind = docKind(absPath);
    ...
    private edit(path: string, rebuild: () => void): void {
        ...
        // Editor only has markdown/yaml/json/plaintext highlighters; tex is plain.
        const filetype = docKind(path) === "markdown" ? "markdown" : "plaintext";
...
export function runBrowser(root: string): void {
    if (findViewableFiles(root).length === 0) {
        process.stderr.write(`No markdown or tex files found under ${root}\n`);
        process.exit(1);
    }
